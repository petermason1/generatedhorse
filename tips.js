...
// Encapsulate the entire script logic in an IIFE (Immediately Invoked Function Expression)
// This creates a function scope, making 'return' statements valid at the top level of this script's logic.
(function() {
    ...
    if (!main) {
        ...
        return; // Valid return inside this IIFE function
    }
    ...
    try {
        if (window.racecardsData && Array.isArray(window.racecardsData.racecards)) {
            ...
            console.warn("tips.js: window.racecardsData or window.racecardsData.racecards is not available or not an array. Displaying 'no races' message.");
            main.innerHTML = `
                <div class="container">
        ...
        main.innerHTML = `
            <div class="container">
                ...
                <p>Please check your 'data.js' file for errors in the browser console.</p>
            </div>
        `;
        return; // Valid return inside this IIFE function
    }
    ...
    // IMPORTANT CHANGE: Ensure each runner gets a reference to its full race object
    races.forEach(race => {
        (race.runners || []).forEach(r => {
            r.score = scoreRunner(r);
            r.oddsDecimal = fractionToDecimalOdds(r.odds?.[0]?.fractional);
            r.race = race; // Explicitly attach the race object to the runner
        });
    });
    ...
    function addPick(pick, categoryArray, maxLimit) {
        // Ensure pick.race is defined before calling runnerKey
        if (!pick.race) {
    ...
    races.forEach(race => {
        ...
        if (!topScorerInRace || topScorerInRace.score <= 0) {
    ...
    console.log(`tips.js: Generated ${topPicks.length} Top Picks, ${valuePicks.length} Value Bets, ${outsiders.length} Outsiders.`);
    ...
    try {
        ...
        main.innerHTML = `
            <div class="container">
                <h1 class="page-title">Today’s Horse Racing Tips</h1>
                <p class="tips-intro">Our advanced system analyzes key performance metrics for every horse and race to identify potential top picks, value bets, and promising outsiders for today's racing action. Remember to always gamble responsibly.</p>
                <h2>An error occurred while displaying tips.</h2>
    ...
    // --- Helper Functions (can be moved to bottom for clarity, or can be in a separate 'utils.js' if shared) ---
