...
function convertLbsToStone(lbs) {
  ...
  let result = '';
  if (stones > 0) result += `${stones}st`;
  if (pounds > 0) result += `${stones > 0 ? ' ' : ''}${pounds}lb`;
  else if (stones === 0 && totalLbs > 0) result += `${totalLbs}lb`;
  return result.trim() || '-';
}
...
function renderTopPicks(race) {
  let top = race.runners
    .filter(r => !isNonRunner(r) && typeof r.score === 'number')
